...
func (k *KubernetesClient) tryKubeconfigAuth() (*rest.Config, error) {
    ...
    if err != nil { return nil, fmt.Errorf("failed to create REST config from kubeconfig: %w", err) }
...
func (k *KubernetesClient) tryInClusterAuth() (*rest.Config, error) {
    ...
    if err != nil { return nil, fmt.Errorf("failed to get in-cluster config: %w", err) }
    if serverURL := k.pluginConfig.GetServerURL(); serverURL != "" {
...
func (k *KubernetesClient) tryDirectAuth() (*rest.Config, error) {
    ...
    if serverURL == "" { return nil, fmt.Errorf("server URL is required for direct authentication") }
...
func (k *KubernetesClient) isRunningInCluster() bool {
    if _, err := os.Stat("/var/run/secrets/kubernetes.io/serviceaccount/token"); err == nil { return true }
    if os.Getenv("KUBERNETES_SERVICE_HOST") != "" && os.Getenv("KUBERNETES_SERVICE_PORT") != "" { return true }
    return false
}
...
func (k *KubernetesClient) TestConnection(ctx context.Context) error {
    ...
    if err != nil { return fmt.Errorf("permission test failed (cannot list namespaces): %w", err) }
    return nil
}
...
func (k *KubernetesClient) GetClientInfo(ctx context.Context) (map[string]interface{}, error) {
    ...
    if version, err := k.clientset.Discovery().ServerVersion(); err == nil { info["server_version"] = version.String() }
    if k.authMethod == AuthMethodKubeconfig {
        if currentContext, err := k.getCurrentContext(); err == nil { info["current_context"] = currentContext }
    }
    return info, nil
}
...
func (k *KubernetesClient) getNamespaces(ctx context.Context) ([]string, error) {
    ...
    if len(namespaceList) == 1 && namespaceList[0] == "*" {
        ...
        if err != nil { return nil, fmt.Errorf("failed to list namespaces: %w", err) }
        var namespaces []string
        for _, ns := range nsList.Items { namespaces = append(namespaces, ns.Name) }
...
func (k *KubernetesClient) ListPods(ctx context.Context, opts internal.ListOptions) ([]corev1.Pod, error) {
    ...
    for _, namespace := range namespaces {
        ...
        if opts.TimeoutSeconds != nil { listOpts.TimeoutSeconds = opts.TimeoutSeconds }
        pods, err := k.clientset.CoreV1().Pods(namespace).List(ctx, listOpts)
        if err != nil { return nil, fmt.Errorf("failed to list pods in namespace %s: %w", namespace, err) }
...
func (k *KubernetesClient) GetPodLogs(ctx context.Context, namespace, podName, containerName string, opts internal.LogOptions) (io.ReadCloser, error) {
    if namespace == "" { return nil, fmt.Errorf("namespace cannot be empty") }
    if podName == "" { return nil, fmt.Errorf("pod name cannot be empty") }
    ...
    if opts.SinceTime != nil { logOpts.SinceTime = opts.SinceTime }
    if opts.SinceSeconds != nil { logOpts.SinceSeconds = opts.SinceSeconds }
    if opts.TailLines != nil { logOpts.TailLines = opts.TailLines }
    if opts.LimitBytes != nil { logOpts.LimitBytes = opts.LimitBytes }
...
func (k *KubernetesClient) ListEvents(ctx context.Context, opts internal.ListOptions) ([]corev1.Event, error) {
    ...
    for _, namespace := range namespaces {
        ...
        if err != nil { return nil, fmt.Errorf("failed to list events in namespace %s: %w", namespace, err) }
...
func (k *KubernetesClient) WatchEvents(ctx context.Context, opts internal.ListOptions) (internal.EventWatcher, error) {
    ...
    if err != nil { return nil, fmt.Errorf("failed to watch events: %w", err) }
...
func (e *eventWatcherAdapter) ResultChan() <-chan internal.EventWatchResult {
    ...
    go func() {
        ...
        for item := range ch {
            var ev *corev1.Event
            if event, ok := item.Object.(*corev1.Event); ok { ev = event }
...
func (k *KubernetesClient) ListResources(ctx context.Context, resourceType string, opts internal.ListOptions) ([]runtime.Object, error) {
    ...
    for _, namespace := range namespaces {
        ...
        case "pod", "pods":
            pods, err := k.clientset.CoreV1().Pods(namespace).List(ctx, listOpts)
            if err != nil { return nil, fmt.Errorf("failed to list pods in namespace %s: %w", namespace, err) }
            for i := range pods.Items { allResources = append(allResources, &pods.Items[i]) }
        case "service", "services", "svc":
            services, err := k.clientset.CoreV1().Services(namespace).List(ctx, listOpts)
            if err != nil { return nil, fmt.Errorf("failed to list services in namespace %s: %w", namespace, err) }
            for i := range services.Items { allResources = append(allResources, &services.Items[i]) }
        case "deployment", "deployments", "deploy":
            deployments, err := k.clientset.AppsV1().Deployments(namespace).List(ctx, listOpts)
            if err != nil { return nil, fmt.Errorf("failed to list deployments in namespace %s: %w", namespace, err) }
            for i := range deployments.Items { allResources = append(allResources, &deployments.Items[i]) }
        case "replicaset", "replicasets", "rs":
            replicasets, err := k.clientset.AppsV1().ReplicaSets(namespace).List(ctx, listOpts)
            if err != nil { return nil, fmt.Errorf("failed to list replicasets in namespace %s: %w", namespace, err) }
            for i := range replicasets.Items { allResources = append(allResources, &replicasets.Items[i]) }
        case "configmap", "configmaps", "cm":
            configmaps, err := k.clientset.CoreV1().ConfigMaps(namespace).List(ctx, listOpts)
            if err != nil { return nil, fmt.Errorf("failed to list configmaps in namespace %s: %w", namespace, err) }
            for i := range configmaps.Items { allResources = append(allResources, &configmaps.Items[i]) }
        case "secret", "secrets":
            secrets, err := k.clientset.CoreV1().Secrets(namespace).List(ctx, listOpts)
            if err != nil { return nil, fmt.Errorf("failed to list secrets in namespace %s: %w", namespace, err) }
            for i := range secrets.Items { allResources = append(allResources, &secrets.Items[i]) }
        default:
...
func (k *KubernetesClient) GetPodsWithLogs(ctx context.Context, opts internal.ListOptions) ([]corev1.Pod, error) {
    ...
    for _, pod := range pods {
        ...
        for _, containerStatus := range pod.Status.ContainerStatuses {
            if containerStatus.State.Running != nil || containerStatus.State.Terminated != nil { hasLogs = true; break }
        }
        if hasLogs { podsWithLogs = append(podsWithLogs, pod) }
    }
