...
import (
    ...
    k8stables "github.com/turbot/tailpipe-plugin-kubernetes/kubernetes"
...
func (s *KubernetesPodLogsSource) Init(ctx context.Context, params *row_source.RowSourceParams, opts ...row_source.RowSourceOption) error {
    ...
    if err := s.RowSourceImpl.Init(ctx, params, opts...); err != nil { return err }
...
func (s *KubernetesPodLogsSource) Collect(ctx context.Context) error {
    ...
    if !from.IsZero() { filters.SinceTime = &from }
    if !to.IsZero() { filters.UntilTime = &to }

    filters.Namespaces = s.Connection.GetNamespaceList()
    if len(s.Config.LogLevels) > 0 { filters.LogLevels = s.Config.LogLevels }
    if s.Config.TailLines != nil { filters.TailLines = s.Config.TailLines }
    if s.Config.LimitBytes != nil { filters.Limit = s.Config.LimitBytes }
    if s.Config.PodNamePattern != "" {
        if re, err := regexp.Compile(s.Config.PodNamePattern); err == nil { filters.PodNamePattern = re }
    }
    ...
    if srcName == "" { srcName = KubernetesPodLogsSourceIdentifier }
    for _, r := range rows {
        ts := time.Time{}
        if r.Timestamp != nil { ts = *r.Timestamp }
        if ts.IsZero() { ts = from }

        id := strings.Join([]string{r.Namespace, r.PodName, r.ContainerName, ts.UTC().Format(time.RFC3339Nano)}, "|")
        if !s.CollectionState.ShouldCollect(id, ts) { continue }

        if err := s.CollectionState.OnCollected(id, ts); err != nil { continue }
        ...
        if err := s.OnRow(ctx, &types.RowData{ Data: r, SourceEnrichment: enrichment }); err != nil { continue }
    }
