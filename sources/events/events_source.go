...
func (s *KubernetesEventsSource) Collect(ctx context.Context) error {
    ...
    if s.Config.MessagePattern != "" {
        if re, err := regexp.Compile(s.Config.MessagePattern); err == nil { filters.MessagePattern = re }
    }
    if s.Config.ReasonPattern != "" {
        if re, err := regexp.Compile(s.Config.ReasonPattern); err == nil { filters.ReasonPattern = re }
    }
    ...
    if srcName == "" { srcName = KubernetesEventsSourceIdentifier }
    for _, r := range rows {
        ts := time.Time{}
        if r.Timestamp != nil { ts = *r.Timestamp }
        if ts.IsZero() {
            if r.LastTimestamp != nil { ts = *r.LastTimestamp } else if r.FirstTimestamp != nil { ts = *r.FirstTimestamp } else { ts = from }
        }
